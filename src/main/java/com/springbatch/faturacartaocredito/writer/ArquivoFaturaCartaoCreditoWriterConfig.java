...
/* 1. Objeto escritor.
*  2. FlatFileItemWriter = escrever em um único arquivo.
*  3. MultiResourceItemWriter = para escrever em múltiplos arquivos.
*  4. Informar o nome dos arquivos =
*  a. prefixo =  files/fatura
*  b. sufixo = index + formato .txt
*  5. lineAggregator = agregador de linha para a lógica de exibição das transações da fatura cartão de crédito
* no arquivo.
*  6. LineAggregator = a lógica do método: cabeçalho(dados do cliente) + iteração(iterar pelas transacoes da fatura
* formatada, informando data, descricao e valor da transacao) + retorno da string
* construída(contendo todas as transações)
*  7. headerCallback e footerCallback = cria cabeçalhos e rodapés no spring.
*  8. headerCallback: faz uso do writer = adição de recursos nas linhas
*  9. footerCallback: tem que ser listener, porque precisará totalizar os registros escritos, ou seja, esperar
* o momento da escrita e totalizar estes registros(colocar .listener em FaturaCartaoCreditoStepConfig) e não
* esquecer que é um component e adicionar o @Bean(método precisa ser public) */
